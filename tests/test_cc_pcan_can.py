...
def test_rename_trc_truncate_filename(tmp_path, mock_can_bus, mock_PCANBasic, caplog, mocker):
    ...
    long_name = "a" * 260 + ".trc"  # 264 characters total, definitely over 255
    expected_truncated = "a" * 251 + ".trc"  # 255 - 4 (for .trc) = 251

    # Create a mock trace file that would exist to be renamed
    ...
    # Mock the Path.rename method to raise OSError for long filename, then succeed for truncated name
    ...
    # Verify rename was called twice: first with long name (fails), then with truncated name (succeeds)
...
def test_rename_trc_oserror_short_filename(tmp_path, mock_can_bus, mock_PCANBasic, caplog, mocker):
    ...
    # Create a short filename (well under 255 characters)
    short_name = "short_filename.trc"  # Only 18 characters, well under limit
    ...
    # Mock the Path.rename method to raise OSError for a different reason (not filename length)
    mock_rename = mocker.patch.object(Path, "rename")
    ...
    # Verify rename was called only once (no retry for truncation)
