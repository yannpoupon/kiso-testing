...
def test_start_can_trace_decorator(mocker: MockerFixture, opened: bool, channel_class: CCPCanCan | CCSocketCan):
    ...
    pcan_mock.start_can_trace.assert_called_once_with(trace_path=pcan_mock.trace_path / trace_name)
    if not opened:
...
@pytest.mark.parametrize("channel_class", (CCPCanCan, CCSocketCan))
def test_stop_can_trace_decorator(mocker: MockerFixture, channel_class: CCPCanCan | CCSocketCan):
    pcan_mock = mocker.MagicMock(spec=channel_class)
    ...
    assert returned_value == "value"
    pcan_mock.stop_can_trace.assert_called_once_with()
