...
from pykiso.lib.auxiliaries.yepkit_auxiliaries.common.yepkit_base import (
    YEPKIT_USB_PACKET_PAYLOAD_SIZE,
    YEPKIT_USB_PACKET_SIZE,
    YEPKIT_USB_TIMEOUT,
    PortNumberError,
    PortState,
    SetStateError,
...
class YkurAuxiliary(YepkitBase):
    ...
    def __init__(self, serial_number: str = None, **kwargs):
        ...
        self.connect_device(serial=serial_number, list_pid=YKUR_USB_PID_LIST)
    ...
    def set_state(self, target: Union[int, str], state: int):
        ...
        """
        self._raw_sendreceive([state, YKUR_TARGET_PORT[target]])

        status = self.get_relay_state(target)
    ...
    def _raw_sendreceive(self, packetarray: List[int]) -> List[int]:
        ...
        """
        self._device.write([0x20])
        size = 1
        packetarray = packetarray + [0x00] * (
            YEPKIT_USB_PACKET_SIZE + size - len(packetarray)
        )

        log.info(f"message send {packetarray}")
        self._device.write(packetarray)
        recvpacket = self._device.read(
            max_length=YEPKIT_USB_PACKET_SIZE + size, timeout_ms=YEPKIT_USB_TIMEOUT
        )
