...
class YkushAuxiliary(YepkitBase):
    ...
    def __init__(self, serial_number: str = None, **kwargs):
        ...
        """
        super().__init__()
        self.find_device(list_pid=YKUSH_USB_PID_LIST, serial=serial_number)
    ...
    def _raw_sendreceive(self, packetarray: List[int]) -> List[int]:
        """Function to send a message to the device and get the response message.

        :param packetarray: packet to send
        :return: the response from the device
        """
        return super()._raw_sendreceive(packetarray=packetarray * 2)
