...
class XrayPublisher:
    ...
    def get_token(self, url: str) -> str:
        ...
        token_result = requests.post(url=url, headers=headers, json=client, timeout=30)
    ...
    def publish_xml_result(
        ...
        if test_execution_name is None:
            return self._publish_xml_result(
                token=token, data=data, project_key=project_key, test_execution_id=test_execution_id, timeout=timeout
            )
    ...
    def _publish_xml_result(
        self, token: str, data: dict, project_key: str, test_execution_id: str | None = None, timeout: float = 30
    ) -> dict[str, str]:
        ...
        try:
            query_response = requests.post(url=url_publisher, headers=headers, data=data, timeout=timeout)
    ...
    def _publish_xml_result_multipart(
        ...
        try:
            query_response = requests.post(url_publisher, headers=headers, files=files, timeout=timeout)
        except requests.exceptions.ConnectionError:
            raise XrayException(f"Cannot connect to JIRA service at {url_endpoint}")
        else:
            query_response.raise_for_status()
...
def extract_test_results(path_results: Path, merge_xml_files: bool) -> dict[str, str]:
    """
    ...
    :return: dict with the name of the xml files parsed and the data parsed
    """
    xml_results = []
